# Package index

## Autoplot Methods

- [`autoplot(`*`<glyexp_experiment>`*`)`](https://glycoverse.github.io/glyvis/dev/reference/autoplot.glyexp_experiment.md)
  : Plots for Experiments
- [`autoplot(`*`<glystats_anova_res>`*`)`](https://glycoverse.github.io/glyvis/dev/reference/autoplot.glystats_anova_res.md)
  [`autoplot(`*`<glystats_kruskal_res>`*`)`](https://glycoverse.github.io/glyvis/dev/reference/autoplot.glystats_anova_res.md)
  : Plots for Multi-Group Differential Expression Analysis (DEA)
- [`autoplot(`*`<glystats_cc_res>`*`)`](https://glycoverse.github.io/glyvis/dev/reference/autoplot.glystats_cc_res.md)
  : Plots for Consensus Clustering
- [`autoplot(`*`<glystats_cor_res>`*`)`](https://glycoverse.github.io/glyvis/dev/reference/autoplot.glystats_cor_res.md)
  : Plots for Correlation Analysis
- [`autoplot(`*`<glystats_cox_res>`*`)`](https://glycoverse.github.io/glyvis/dev/reference/autoplot.glystats_cox_res.md)
  : Plots for Cox Proportional Hazards Model
- [`autoplot(`*`<glystats_go_ora_res>`*`)`](https://glycoverse.github.io/glyvis/dev/reference/autoplot.glystats_go_ora_res.md)
  [`autoplot(`*`<glystats_kegg_ora_res>`*`)`](https://glycoverse.github.io/glyvis/dev/reference/autoplot.glystats_go_ora_res.md)
  [`autoplot(`*`<glystats_reactome_ora_res>`*`)`](https://glycoverse.github.io/glyvis/dev/reference/autoplot.glystats_go_ora_res.md)
  : Plots for Enrichment Analysis
- [`autoplot(`*`<glystats_hclust_res>`*`)`](https://glycoverse.github.io/glyvis/dev/reference/autoplot.glystats_hclust_res.md)
  : Plots for Hierarchical Clustering
- [`autoplot(`*`<glystats_limma_res>`*`)`](https://glycoverse.github.io/glyvis/dev/reference/autoplot.glystats_limma_res.md)
  : Plots for Limma Result
- [`autoplot(`*`<glystats_oplsda_res>`*`)`](https://glycoverse.github.io/glyvis/dev/reference/autoplot.glystats_oplsda_res.md)
  : Plots for OPLS-DA
- [`autoplot(`*`<glystats_pca_res>`*`)`](https://glycoverse.github.io/glyvis/dev/reference/autoplot.glystats_pca_res.md)
  : Plots for Principal Component Analysis (PCA)
- [`autoplot(`*`<glystats_plsda_res>`*`)`](https://glycoverse.github.io/glyvis/dev/reference/autoplot.glystats_plsda_res.md)
  : Plots for PLS-DA
- [`autoplot(`*`<glystats_roc_res>`*`)`](https://glycoverse.github.io/glyvis/dev/reference/autoplot.glystats_roc_res.md)
  : Plots for ROC Analysis
- [`autoplot(`*`<glystats_tsne_res>`*`)`](https://glycoverse.github.io/glyvis/dev/reference/autoplot.glystats_tsne_res.md)
  : Plots for t-SNE
- [`autoplot(`*`<glystats_ttest_res>`*`)`](https://glycoverse.github.io/glyvis/dev/reference/autoplot.glystats_ttest_res.md)
  [`autoplot(`*`<glystats_wilcox_res>`*`)`](https://glycoverse.github.io/glyvis/dev/reference/autoplot.glystats_ttest_res.md)
  : Plots for 2-Group Differential Expression Analysis (DEA)
- [`autoplot(`*`<glystats_umap_res>`*`)`](https://glycoverse.github.io/glyvis/dev/reference/autoplot.glystats_umap_res.md)
  : Plots for UMAP
- [`autoplot(`*`<glystats_wgcna_res>`*`)`](https://glycoverse.github.io/glyvis/dev/reference/autoplot.glystats_wgcna_res.md)
  : Plots for WGCNA

## Direct Plot

- [`plot_boxplot()`](https://glycoverse.github.io/glyvis/dev/reference/plot_boxplot.md)
  : Boxplot
- [`plot_corrplot()`](https://glycoverse.github.io/glyvis/dev/reference/plot_corrplot.md)
  : Correlation Plot
- [`plot_enrich()`](https://glycoverse.github.io/glyvis/dev/reference/plot_enrich.md)
  : Plot Enrichment Analysis Results
- [`plot_heatmap()`](https://glycoverse.github.io/glyvis/dev/reference/plot_heatmap.md)
  : Heatmap
- [`plot_logo()`](https://glycoverse.github.io/glyvis/dev/reference/plot_logo.md)
  : Logo Plot
- [`plot_oplsda()`](https://glycoverse.github.io/glyvis/dev/reference/plot_oplsda.md)
  : OPLS-DA Plot
- [`plot_pca()`](https://glycoverse.github.io/glyvis/dev/reference/plot_pca.md)
  : PCA Plot
- [`plot_plsda()`](https://glycoverse.github.io/glyvis/dev/reference/plot_plsda.md)
  : PLS-DA Plot
- [`plot_roc()`](https://glycoverse.github.io/glyvis/dev/reference/plot_roc.md)
  : ROC plot
- [`plot_tsne()`](https://glycoverse.github.io/glyvis/dev/reference/plot_tsne.md)
  : t-SNE Plot
- [`plot_umap()`](https://glycoverse.github.io/glyvis/dev/reference/plot_umap.md)
  : UMAP Plot
- [`plot_volcano()`](https://glycoverse.github.io/glyvis/dev/reference/plot_volcano.md)
  : Volcano Plot
